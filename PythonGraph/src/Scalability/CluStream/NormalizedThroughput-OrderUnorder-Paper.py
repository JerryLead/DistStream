...
rects1 = plt.bar(pos, p_8, width, color='white', label=U"并行度=8", edgecolor='black')

rects3 = plt.bar([p + width for p in pos], p_16, width,  color='white', label=U"并行度=16", hatch='///', edgecolor='black')

rects2 = plt.bar([p + width * 2 for p in pos], p_32, width,  color='white',label=U"并行度=32", hatch='\\\\\\', edgecolor='black')


plt.ylabel(U'归一化吞吐率', weight='medium')
...
plt.show()
#plt.savefig(dir + "NormalizedThroughPut-CluStream-8&16&32-Paper.pdf")
