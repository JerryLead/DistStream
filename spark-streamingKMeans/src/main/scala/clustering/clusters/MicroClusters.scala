...
case class MicroClusters(
                          val microclusters: Array[MicroCluster],
                          val horizonWindowSize: Int = 1000,
                          val clusterRadius: Double = 2.0,
                          val m: Int = 100
                        ) extends Clusters {
  ...
  override def update(change: Example): MicroClusters = {
    ...
    val radius = clusterRadius * microclusters(closest).rmse
    if(distance<=radius) {
      ...
      val threshold = timestamp - horizonWindowSize
      var i: Int = 0
