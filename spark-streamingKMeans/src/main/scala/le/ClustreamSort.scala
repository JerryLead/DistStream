...
import clustreamwithSort.{ClustreamOnlineWithSort, ClustreamWithSort}
import core.{DenseInstance, Instance, NullInstance,Example}
...
object ClustreamSort {
  ...
  def main(args: Array[String]) {
    ...
    /*val kafkaParam = Map[String,Object] ("bootstrap.servers" -> "133.133.20.8:9092",
      "key.deserializer" -> classOf[StringDeserializer],
      "value.deserializer" -> classOf[StringDeserializer],
      "group.id" -> "CluStreamExample",
      "auto.offset.reset" -> "latest",
    ...
    val model = new ClustreamOnlineWithSort(q,numDimensions,0,batchTime)
    ...
    val clustream = new ClustreamWithSort(model)
    ...
    System.out.println("Cls size:" + cls.length)
