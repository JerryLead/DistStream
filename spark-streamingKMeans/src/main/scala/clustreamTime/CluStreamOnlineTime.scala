package clustreamTime

import java.io.{FileInputStream, ObjectInputStream}

import breeze.linalg._
import breeze.stats.distributions.Gaussian
import clustreamTime.MicroCluster
...
class CluStreamOnlineTime(
    ...
    data.foreachRDD { (rdd, timeS) =>
      ...
      if (!rdd.isEmpty()) {
        if (initialized) {
          ...
          /*for(mc <- broadcastMic.value){
            System.out.println("微簇时间戳" + mc._1.cf1t)
            System.out.println("微簇权重" + mc._1.getN)
            System.out.println("微簇时间" + mc._1.getCf1t / mc._1.getN)
            System.out.println("====")
          }*/
  ...
  private def addPointMicroClusters(idx1: Int, point: (Vector[Double])): Unit = {
    ...
    microClusters(idx1).setCf1t(microClusters(idx1).getCf1t + time)
    microClusters(idx1).setCf2t(microClusters(idx1).getCf2t + (time * time))
  ...
  private def replaceMicroCluster(idx: Int, point: (Vector[Double])): Unit = {
    ...
    microClusters(idx) = new MicroCluster(point :* point, point, time * time, time, 1L, this.tFactor)
  ...
  private def updateMicroClusters(assignations: RDD[(Int, (Long, Vector[Double]))]): Unit = {
    ...
    val aggregateFuntion = (aa: (Vector[Double], Vector[Double], Long, Long, Long, Long), bb: (Vector[Double], Vector[Double], Long, Long, Long, Long))
    => (aa._1 :+ bb._1, aa._2 :+ bb._2, aa._3 + bb._3, aa._4 + bb._4, aa._5 + bb._5, math.max(aa._6,bb._6))
    // sumsAndSumsSquares -> (key: Int, (sum: Vector[Double], sumSquares: Vector[Double], count: Long ) )
    val sumsAndSumsSquares =
      dataIn.mapValues(a => (a._2, a._2 :* a._2, a._1, a._1 * a._1, 1L, a._1)).
        aggregateByKey(Vector.fill[Double](numDimensions)(0.0), Vector.fill[Double](numDimensions)(0.0), 0L, 0L, 0L, 0L)(aggregateFuntion, aggregateFuntion)
        .collect()//coalesce(1).collect()

    assignations.unpersist()

    //if(outliers != null && outliers.length > 35000)
    ...
    for (ss <- sumsAndSumsSquares) {
      ...
      //if (this.time < ss._2._6)
        //this.time = ss._2._6
      microClusters(i).setCf1x(microClusters(i).cf1x :+ ss._2._1)
      microClusters(i).setCf2x(microClusters(i).cf2x :+ ss._2._2)
      microClusters(i).setN(microClusters(i).n + ss._2._5)
      //mc.setCf1t(mc.cf1t + ss._2._3)
      //mc.setCf2t(mc.cf2t + ss._2._4)
      microClusters(i).setCf1t(microClusters(i).cf1t + ss._2._5 * time)
      microClusters(i).setCf2t(microClusters(i).cf2t + ss._2._5 * ( time * time ))
      //System.out.println("加入的权重" + ss._2._5)
      totalIn += ss._2._5
    ...
    * */
    ...
    if (outliers != null && outliers.length != 0) {
      ...
      var oo = outliers.map(x => x._2).toList
      //oo = Random.shuffle(oo)
        i = 0
        for(i  <- (0 until outliers.length).reverse){
        //for(i <- 0 to oo.length -1 ){
        //val point = outliers(i)
        val point = oo(i)
        var minDist = Double.PositiveInfinity
        var idMinDist = 0
        if (recursiveOutliersRMSDCheck == 1) for (id <- newMC) {
          ...
          //println(newMC.length)
          //val dist = squaredDistPointToMCArrIdx(id, point._2)
          val dist = squaredDistPointToMCArrIdx(id, point)
          if (dist < minDist) {
            minDist = dist
            idMinDist = id
          }
        ...
        if (scala.math.sqrt(minDist) <= microClusters(idMinDist).getRMSD) {
          ...
          newMC = newMC :+ safeDeleteMC(j)._1
          j += 1
          //if(j >= safeDeleteMC.length && keepOrMergeMC.length <= 1)
            //j = 0
        }
        else {
          var minDist = Double.PositiveInfinity
...
class MicroCluster(
  ...
  def getRMSD: Double = { //TODO: 忽略了n=1的情况
    ...
    //return this.tfactor * scala.math.sqrt(sum(this.cf2x) / this.n.toDouble - sum(this.cf1x.map(a => a * a)) / (this.n.toDouble * this.n.toDouble))
    else{
    }
