...
public class CMM_GTAnalysis implements Serializable{
     ...
     */
    private double tauConnection = 1.0;
    ...
    public HashMap<Integer, HashMap<Integer, Double>> pointsToClusterConnMap = new HashMap<>();
     ...
     *
     */
    protected class CMMPoint extends DataPoint implements Serializable{
        /**
     ...
     *
     */
    protected class GTCluster implements Serializable{
        static final long serialVersionUID = 311739925877369930L;
...
//                    System.out.println("new autoExpandVector spends " + (System.currentTimeMillis() - start));
                    //calculate nearest neighbours
                    getKnnInCluster(cmdp, knnNeighbourhood, points, knnDist,knnPointIndex);
//                    System.out.println("get Knn In Cluster spends " + (System.currentTimeMillis() - start));
...
//                    System.out.println("avgKnn + spends " + (System.currentTimeMillis() - start));
                    if(knnDist.size()!=0)
                        avgKnn/=knnDist.size();
...
//                if(count % 5000 == 0) {
//                    System.out.println("calculateKnn point number " + count);
//                    System.out.println("time spends " + (System.currentTimeMillis() - start));
//                }
//                count++;
            }
        ...
        private void calculateKnnFromFile() {
//            long start = System.currentTimeMillis();
//            int count = 0;
            for (int p0 : points) {
                CMMPoint cmdp = cmmpoints.get(p0);
                if(!cmdp.isNoise()){
                    knnMeanAvg+=cmdp.knnInCluster;
                    knnDevAvg+=Math.pow(cmdp.knnInCluster,2);
                }
//                if(count % 5000 == 0) {
//                    System.out.println("calculateKnn point number " + count);
//                    System.out.println("time spends " + (System.currentTimeMillis() - start));
...
//                    if(!pointsToClusterConnMap.containsKey(p))
//                        pointsToClusterConnMap.put(p, new HashMap<Integer, Double>());
//                    pointsToClusterConnMap.get(p).put(otherCid, con_p_Cj);
        ...
        private void calculateClusterConnectionFromFile(int otherCid, boolean initial){
            double avgConnection = 0;
            if(workclass==otherCid){
                avgConnection = 1;
            }
                ...
                for(int p : points){
                    CMMPoint cmdp = cmmpoints.get(p);
                    double con_p_Cj = pointsToClusterConnMap.get(p).get(otherCid);
                    double connection = cmdp.connectivity * con_p_Cj;
                    if(initial){
                        cmdp.p.setMeasureValue("Connection to C"+otherCid, con_p_Cj);
                    }
     ...
     */
    public CMM_GTAnalysis(Clustering trueClustering, ArrayList<DataPoint> points, boolean enableClassMerge, String middleResultPath){
        if(debug)
        ...
        long start = System.currentTimeMillis();
        System.out.println("start calculateKnn");
        //calculate initial knnMean and knnDev
        for (GTCluster gtc : gt0Clusters) {
            gtc.calculateKnn();
        }
//        saveObjectToFile(middleResultPath + "points", cmmpoints);
//        ArrayList<CMMPoint> temp = new ArrayList<>(cmmpoints);
//        cmmpoints = (ArrayList<CMMPoint>)(readObjectFromFile(middleResultPath + "points"));
//        for (GTCluster gtc : gt0Clusters) {
//            gtc.calculateKnnFromFile();
//        }
//        for(int i = 0; i < temp.size(); i++) {
//            double[] original = temp.get(i).toDoubleArray();
//            double[] read = cmmpoints.get(i).toDoubleArray();
//            for (int j = 0; j < original.length; j++) {
//                if (original[j] != read[j])
//                    System.out.println("false!!!!!!!!!!!!!!!!");
//            }
//            if(temp.get(i).knnInCluster != cmmpoints.get(i).knnInCluster)
//                System.out.println("false again!!!!!!!!!!!!!!!!");
//        }
        long end = System.currentTimeMillis();
        System.out.println("finish calculateKnn " + (end - start) + "ms");

        start = System.currentTimeMillis();
        System.out.println("start calculateGTClusterConnections");
        //calculate cluster connections
        calculateGTClusterConnections();
//        saveObjectToFile(middleResultPath + "connections", pointsToClusterConnMap);
//        HashMap<Integer, HashMap<Integer, Double>> tempMap = new HashMap(pointsToClusterConnMap);
//        pointsToClusterConnMap = (HashMap<Integer, HashMap<Integer, Double>>)(readObjectFromFile(middleResultPath + "connections"));
//        calculateGTClusterConnections();
//        for(int point : pointsToClusterConnMap.keySet()) {
//            for (int cluster : pointsToClusterConnMap.get(point).keySet()) {
//                if(Math.abs(pointsToClusterConnMap.get(point).get(cluster) - tempMap.get(point).get(cluster)) > 1e-50)
//                    System.out.println("false triple!!!!!!!!!!!!!!!! read: " + pointsToClusterConnMap.get(point).get(cluster) + " original: " + tempMap.get(point).get(cluster));
//            }
//        }

        end = System.currentTimeMillis();
        System.out.println("finish calculateGTClusterConnections " + (end - start) + "ms");

        start = System.currentTimeMillis();
        System.out.println("start calculateGTPointQualities");
        //calculate point connections with own clusters
        calculateGTPointQualities();
        end = System.currentTimeMillis();
        System.out.println("finish calculateGTPointQualities " + (end - start) + "ms");
...
//            System.out.println("cal distance time: " + (System.currentTimeMillis() - start));
//            start = System.currentTimeMillis();
...
//            System.out.println("adjust time: " + (System.currentTimeMillis() - start));
        }
//        System.out.println("getKnnInCluster time: " + (System.currentTimeMillis() - start));
    }
...
//                System.out.println("finish calculate cluster" + c0 + " to cluster" + c1 + " connection, spends " + (System.currentTimeMillis() - start) + "ms");
//                gt0Clusters.get(c0).calculateClusterConnectionFromFile(c1, true);
            }
        }
...
//            return 1-mergedConn/(double)totalConn;
    ...
    private void saveObjectToFile(String dataPath, Object object) {
        ...
        try {
            ...
            System.out.println("saveToFile done. spends " + (System.currentTimeMillis() - start) + "ms");
        } catch (FileNotFoundException e) {
            System.out.println("file " + dataPath + " not found when saveToFile");
            e.printStackTrace();
        } catch (IOException e) {
            System.out.println("oos error when saveToFile");
            e.printStackTrace();
        } finally {
            try {
                oos.close();
                fos.close();
                System.out.println("close fos and oos when saveToFile");
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    private Object readObjectFromFile(String dataPath) {
        ...
        try {
            ...
            System.out.println("read done. spends " + (System.currentTimeMillis() - start) + "ms");
            return object;
        } catch (FileNotFoundException e) {
            System.out.println("file " + dataPath + " not found when read");
            e.printStackTrace();
        } catch (ClassNotFoundException e) {
            System.out.println("classNotFound when read");
            e.printStackTrace();
        } catch (IOException e) {
            System.out.println("oos error when read");
            e.printStackTrace();
        }
        return null;
    }
